...
*   **出力データ**:
    ...
    *   **展開図リスト**: 重なり（自己交差）のないすべての有効な展開図パターンのJSONリスト。各パターンには以下のデータが含まれます。
        *   **評価指標 (Metrics)**: 仮に外接矩形（バウンディングボックス）の面積、外周長、縦横比とする
        *   **展開図構成データ (JSON)**:
...
## 2. 展開図生成アルゴリズムの設計
既存のプログラム[fifth.py]をライブラリとして呼び出すため省略

## 3. データ形式の定義
既存のプログラム[fifth.py]に実装済みのため省略。

要約すると面の形状をグラフとして表現する。

そのグラフをノード、辺をエッジとしたグラフで立体を表現する。
